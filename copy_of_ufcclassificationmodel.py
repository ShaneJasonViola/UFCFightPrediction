# -*- coding: utf-8 -*-
"""Copy of ufcclassificationmodel.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1u0FjJRvsxofljxXwMGmaWsQX6ASxqXAM
...
# KO %
if all(col in df.columns for col in ['RedWinsByKO', 'RedWins']):
    df['RedKOPct'] = df['RedWinsByKO'] / (df['RedWins'] + 1)

if all(col in df.columns for col in ['BlueWinsByKO', 'BlueWins']):
    df['BlueKOPct'] = df['BlueWinsByKO'] / (df['BlueWins'] + 1)

df['KOPctDiff'] = df.get('RedKOPct', 0) - df.get('BlueKOPct', 0)
...
# Create WinnerBinary (1 = Fighter A wins, 0 = Fighter B wins) BEFORE flipping
df['WinnerBinary'] = df['Winner'].map({'Red': 1, 'Blue': 0})

# Identify all Red and Blue columns
...
# Create a mask to randomly select 50% of the rows to flip
flip_mask = np.random.rand(len(df)) < 0.5

# Flip Red and Blue columns
...
# Flip the WinnerBinary label for the flipped rows
df.loc[flip_mask, 'WinnerBinary'] = 1 - df.loc[flip_mask, 'WinnerBinary']

# Drop the original Winner column (if no longer needed)
df.drop(columns=['Winner'], inplace=True)

# Check class balance
print(df['WinnerBinary'].value_counts())
...
features = [
    ...
    'RedAvgSubAtt', 'SubAttDif', 'RedWinsBySubmission', 'ReachDif', 'WinnerBinary',
    'RedAvgSigStrLanded', 'SubPctDiff', 'AgeDif', 'BlueTotalFights',
    'RedWins', 'ReachDif', 'TDPctDiff', 'SigStrPctDif', 'KOPctDiff',
...
sns.heatmap(cm_rf, annot=True, fmt='d', cmap='Blues', xticklabels=['Loss', 'Win'], yticklabels=['Loss', 'Win'])
...
# Save using compatible pickle protocol
...
# Re-run data loading and preprocessing steps to define df
...
# Now the original selected cell should work
print(df.columns)